...
# --- NEW: Isolated Mini-Thread Memory ---
# These functions handle the completely separate "mini agent" memory space

async def save_mini_thread(memory_data: dict):
    """
    Saves a mini-thread to a separate, isolated namespace in Pinecone.
    This keeps the mini-agent interactions completely separate from main chat.
    
    Args:
        memory_data: Dictionary with user_id, snippet, query, response, timestamp
    """
    if not pinecone_index:
    ...
    try:
        ...
        # We embed the query + snippet to make the thread searchable later
        text_to_embed = f"Snippet: {memory_data['snippet']}\nQuery: {memory_data['query']}"
        embedding = await ai_clients.get_embedding(text_to_embed)
        ...
        pinecone_index.upsert(
            ...
            namespace="mini_threads"  # <-- THE KEY TO ISOLATION!
        )
        print(f"Mini-thread saved to isolated namespace for user {memory_data['user_id']}")
    except Exception as e:
        print(f"Error saving mini-thread: {e}")


async def get_recent_mini_threads(user_id: str) -> str:
    """
    Fetches all recent mini-threads from the isolated namespace.
    Used by memory consolidation to learn from user's focused questions.
    
    Args:
        user_id: The user's ID
        
    Returns:
        A transcript string of all mini-threads
    """
    if not pinecone_index:
        return ""
    ...
    try:
        ...
        query_embedding = await ai_clients.get_embedding("user query")
        
        results = pinecone_index.query(
            vector=query_embedding,
            top_k=100,  # Get last 100 mini-threads
            filter={"user_id": {"$eq": user_id}},
            namespace="mini_threads"  # <-- Read from the isolated namespace
        )
        
        # Combine into a simple transcript of doubts
        transcript = "\n".join(
            f"Snippet: {m['metadata'].get('snippet')}\nQuery: {m['metadata'].get('query')}\nResponse: {m['metadata'].get('response')}"
            for m in results['matches']
        )
        print(f"Fetched {len(results['matches'])} mini-threads for user {user_id}")
        return transcript
    except Exception as e:
        print(f"Error fetching mini-threads: {e}")
        return ""
